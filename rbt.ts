...
namespace rbt {
    ...
    * Reads the number
    */
    //% weight=30 blockId="gatorlight_light" block="Get light on pin %pin | in %rbt"
    export function light(pin: AnalogPin, type: rbt): number{
      let ADCVal = pins.analogReadPin(pin)
      switch(type){
        case rbt.Lux: return getLux(ADCVal)
        case rbt.adcVal: return ADCVal
